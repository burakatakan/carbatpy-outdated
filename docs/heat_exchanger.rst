...
in properties along the heat exchanger are calculated using a boundary value problem solver.
For the counterflow heat exchanger one shell and a number of inner tubes can be defined.
One class is defined to read the input parameters from a YAML-File and to 
pass them to the heat excahnger class. The results are plotted and stored (also to an Excel file).
